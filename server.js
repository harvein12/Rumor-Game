...
// ============================================================================
// 🔗 CONEXIÓN DE TODAS LAS CARPETAS Y ARCHIVOS (RUMOR Y EXOTIC)
// ============================================================================

// 1. Conectar RUMOR GAME (Archivos en la raíz "/")
app.use(express.static(path.join(__dirname, 'games', 'Rumor Game', 'public')));

// 2. Conectar EXOTIC GAME (Archivos en la ruta "/exotic")
app.use('/exotic', express.static(path.join(__dirname, 'games', 'Exotic Game', 'public')));

// 3. Conectar la carpeta SRC de Exotic (Para que lea main.js y assets/styles.css)
app.use('/exotic/src', express.static(path.join(__dirname, 'games', 'Exotic Game', 'src')));
app.use('/src', express.static(path.join(__dirname, 'games', 'Exotic Game', 'src')));
...
io.on('connection', (socket) => {
    ...
    function progresarMision(usuario, tipoKey, targetSocket) {
        db.get("SELECT xp, misiones, items FROM usuarios WHERE usuario = ?", [usuario], (err, row) => {
            ...
            if (mis[tipoKey] && mis[tipoKey].count < mis[tipoKey].max) {
                mis[tipoKey].count++;
                let earnedXP = 0; if (mis[tipoKey].count === mis[tipoKey].max) { earnedXP = mis[tipoKey].xp; }
                let newXp = row.xp + earnedXP; let newItems = row.items; let alertarDesbloqueo = null;
    ...
    socket.on('loginGoogle', (data) => {
        db.get("SELECT * FROM usuarios WHERE email = ?", [data.email], (err, row) => {
            ...
            else { socket.emit('pedirNombreGoogle', { email: data.email, foto: data.foto }); }
        });
    });
    ...
    socket.on('obtenerSalas', () => { 
        const lista = Object.keys(salas).map(id => ({ 
            ...
            tipoJuego: salas[id].tipoJuego || 'rumor'
        })); 
    ...
    // PARCHE DE RUTAS PARA EVITAR ERRORES EN LAS NUEVAS VERSIONES DE NODE Y EXPRESS
    app.use((req, res, next) => {
        ...
        // Ignorar peticiones de archivos reales para que no mande el HTML por error
        if (req.url.includes('.')) return next();

        // Si es Exotic, forzar su index.html
        if (req.url.startsWith('/exotic')) {
            return res.sendFile(path.join(__dirname, 'games', 'Exotic Game', 'public', 'index.html'));
        }

        // Si no, forzar el index de Rumor
