...
db.serialize(() => {
    // Tabla principal de usuarios (guarda stats, inventario, monedas, pase de batalla)
    db.run(`CREATE TABLE IF NOT EXISTS usuarios (usuario TEXT PRIMARY KEY, email TEXT, pass TEXT, victorias INTEGER DEFAULT 0, avatar TEXT DEFAULT '👤', avatar_type TEXT DEFAULT 'emoji', coins INTEGER DEFAULT 0, toxic INTEGER DEFAULT 0, items TEXT DEFAULT 'base_1', vip INTEGER DEFAULT 0, titulo TEXT DEFAULT '', marco TEXT DEFAULT '', entrada TEXT DEFAULT '', xp INTEGER DEFAULT 0, misiones TEXT DEFAULT '{}')`);
    
    // Fallbacks por si la base de datos es antigua y le faltan columnas nuevas
    ...
    db.run("ALTER TABLE usuarios ADD COLUMN entrada TEXT DEFAULT ''", (err) => {}); 
    db.run("ALTER TABLE usuarios ADD COLUMN xp INTEGER DEFAULT 0", (err) => {}); 
    db.run("ALTER TABLE usuarios ADD COLUMN misiones TEXT DEFAULT '{}'", (err) => {}); 
    
    // Tablas secundarias: Amigos, Historial de partidas y Tickets de Soporte
    ...
    // Crear cuentas maestras por defecto si no existen
    ...
    db.run(`INSERT OR IGNORE INTO usuarios (usuario, email, pass, victorias, avatar, coins, toxic, items, vip, titulo, marco, entrada) VALUES ('soporte1', 'soporte@toxic.com', '123456', 0, '🛡️', 0, 0, '${todos}', 1, 'Soporte Técnico', 'marco-neon', 'e_rayos')`);
});

// Variables globales del servidor en memoria
let salas = {}; // Almacena todas las salas activas
let usuariosConectados = {}; // Mapea socket.id -> nombre de usuario
let usuariosOnline = {}; // Mapea nombre de usuario -> socket.id

// ============================================================================
// ==================== LÓGICA DE SOCKET.IO (TIEMPO REAL) =====================
// ============================================================================
io.on('connection', (socket) => {
    // Variables de sesión exclusivas para este usuario conectado
    let miUsuario = null; 
    let miSalaID = null; 
    let soyEspectador = false;

    // --- SISTEMA DEL PASE TÓXICO Y MISIONES ---
    // Inicia o repara la estructura JSON de las misiones del usuario
    function initMisiones(row) {
        let mis = {};
        if (row && row.misiones) { try { mis = JSON.parse(row.misiones); } catch(e) {} }
        if (!mis || !mis.victorias) {
            mis = { victorias: { count: 0, max: 3, xp: 100 }, comodines: { count: 0, max: 5, xp: 50 }, rumores: { count: 0, max: 10, xp: 150 } };
            if(row && row.usuario) { db.run("UPDATE usuarios SET misiones = ? WHERE usuario = ?", [JSON.stringify(mis), row.usuario]); }
        }
        return mis;
    }

    // Aumenta el progreso de una misión específica y da XP si se completa
    function progresarMision(usuario, tipoKey, targetSocket) {
        db.get("SELECT xp, misiones, items FROM usuarios WHERE usuario = ?", [usuario], (err, row) => {
            if(!row) return; let mis = initMisiones(row);
            if (mis[tipoKey] && mis[tipoKey].count < mis[tipoKey].max) {
                mis[tipoKey].count++;
                let earnedXP = 0; if (mis[tipoKey].count === mis[tipoKey].max) { earnedXP = mis[tipoKey].xp; } // Dio la vuelta, gana XP
                
                let newXp = row.xp + earnedXP; 
                let newItems = row.items; 
                let alertarDesbloqueo = null;
                
                // Chequeo de recompensas por nivel de XP
                if (newXp >= 100 && !newItems.includes('m_legend')) { newItems += ',m_legend'; alertarDesbloqueo = "¡MARCO LEGENDARIO DESBLOQUEADO!"; }
                if (newXp >= 300 && !newItems.includes('tit_dios')) { newItems += ',tit_dios'; alertarDesbloqueo = "¡TÍTULO 'DIOS TÓXICO' DESBLOQUEADO!"; }

                // Guardar en BD y avisar al cliente
                db.run("UPDATE usuarios SET misiones = ?, xp = ?, items = ? WHERE usuario = ?", [JSON.stringify(mis), newXp, newItems, usuario], () => {
                    if (targetSocket) {
                        io.to(targetSocket).emit('updatePase', { misiones: mis, xp: newXp, items: newItems.split(',') });
                        if (earnedXP > 0) io.to(targetSocket).emit('notificacion', `⭐ Misión Completada: +${earnedXP} XP`);
                        if (alertarDesbloqueo) io.to(targetSocket).emit('notificacion', `🌟 ${alertarDesbloqueo}`);
                    }
                });
            }
        });
    ...
    socket.on('solicitarPase', () => {
        if (!miUsuario) return;
        db.get("SELECT xp, misiones, items FROM usuarios WHERE usuario = ?", [miUsuario], (err, row) => {
            if (row) { let mis = initMisiones(row); socket.emit('updatePase', { misiones: mis, xp: row.xp || 0, items: row.items.split(',') }); }
        });
    });
    ...
    socket.on('registro', (data) => {
        ...
        db.get("SELECT email FROM usuarios WHERE email = ?", [data.email], (err, row) => {
            ...
            db.get("SELECT usuario FROM usuarios WHERE LOWER(usuario) = LOWER(?)", [userTrimmed], (err2, row2) => {
                ...
                const misionesDefault = JSON.stringify({ victorias: {count:0, max:3, xp:100}, comodines: {count:0, max:5, xp:50}, rumores: {count:0, max:10, xp:150} });
                const query = `INSERT INTO usuarios (usuario, email, pass, coins, toxic, avatar, avatar_type, items, vip, titulo, marco, entrada, xp, misiones) VALUES (?, ?, ?, 0, 0, '👤', 'emoji', 'base_1', 0, '', '', '', 0, ?)`;
                db.run(query, [userTrimmed, data.email, data.pass, misionesDefault], function(err3) {
                    if (err3) socket.emit('errorLogin', '⚠️ Error al crear la cuenta.'); else iniciarSesion(socket, userTrimmed);
                });
            });
        });
    });
    ...
    // Login mediante Google SSO
    socket.on('loginGoogle', (data) => {
        db.get("SELECT * FROM usuarios WHERE email = ?", [data.email], (err, row) => {
            if (row) { iniciarSesion(socket, row.usuario); } // Ya existe, lo logueamos
            else { socket.emit('pedirNombreGoogle', { email: data.email, foto: data.foto }); } // Nuevo usuario, pedir nombre
        });
    });

    // Terminar registro de Google guardando el nombre elegido
    socket.on('completarRegistroGoogle', (data) => {
        const userTrimmed = String(data.usuario).trim();
        if(!userTrimmed || !data.email) return socket.emit('errorNombreGoogle', '⚠️ Ingresa un nombre válido.');
        db.get("SELECT usuario FROM usuarios WHERE LOWER(usuario) = LOWER(?)", [userTrimmed], (err, row) => {
            if (row) return socket.emit('errorNombreGoogle', '⚠️ El nombre ya está en uso. Elige otro.');
            let avatarUrl = data.foto || '👤'; let avatarType = data.foto ? 'url' : 'emoji';
            const misionesDefault = JSON.stringify({ victorias: {count:0, max:3, xp:100}, comodines: {count:0, max:5, xp:50}, rumores: {count:0, max:10, xp:150} });
            const query = `INSERT INTO usuarios (usuario, email, pass, coins, toxic, avatar, avatar_type, items, vip, titulo, marco, entrada, xp, misiones) VALUES (?, ?, ?, 0, 0, ?, ?, 'base_1', 0, '', '', '', 0, ?)`;
            db.run(query, [userTrimmed, data.email, 'login_google_sso', avatarUrl, avatarType, misionesDefault], function(err2) {
                if (err2) socket.emit('errorNombreGoogle', '⚠️ Error al crear la cuenta.'); else iniciarSesion(socket, userTrimmed);
            });
        });
    });

    // Reconexión automática de Google
    socket.on('reconectarGoogle', (email) => { db.get("SELECT * FROM usuarios WHERE email = ?", [email], (err, row) => { if (row) iniciarSesion(socket, row.usuario); }); });

    // --- FUNCIÓN CENTRAL DE INICIO DE SESIÓN ---
    function iniciarSesion(socket, usuario) {
        miUsuario = usuario; 
        usuariosConectados[socket.id] = usuario; 
        usuariosOnline[usuario] = socket.id;
        
        // Verifica si estaba en una sala antes de desconectarse o recargar la página
        ...
        db.get("SELECT * FROM usuarios WHERE usuario = ?", [usuario], (err, row) => {
            db.all("SELECT resultado, fecha FROM historial WHERE usuario = ? ORDER BY id DESC LIMIT 10", [usuario], (eh, history) => {
                let mis = initMisiones(row);
                // Enviar datos al frontend
                socket.emit('loginExitoso', { ...row, misionesObj: mis, historial: history || [], esAdmin: esAdmin });
                socket.emit('datosCatalogo', CATALOGO);

                // Lógica de Reconexión a Sala/Partida en curso
                if (salaReconectar) {
                    miSalaID = salaReconectar.id;
                    socket.join(salaReconectar.id);
                    let j = salaReconectar.jugadores.find(x => x.nombre === usuario);
                    if (j) j.id = socket.id; // Actualiza el socketID
                    
                    socket.emit('entradoEnSala', { nombreSala: salaReconectar.nombre, codigo: salaReconectar.id, meta: salaReconectar.metaPuntos, rol: 'jugador', privada: !!salaReconectar.pass, tipoJuego: salaReconectar.tipoJuego });
                    
                    if (salaReconectar.juegoEnCurso) {
                        if (salaReconectar.tipoJuego === 'exotic') {
                            socket.emit('faseExotic', { msj: "Reconectado a la mesa de Exotic." });
                        } else {
                            ...
                            // Devolver a la fase de votación o escritura según corresponda
                            if (salaReconectar.timerVotacion && salaReconectar.rumores.length >= salaReconectar.jugadores.length - 1) {
                                ...
                                let retoPicante = "";
                                if (salaReconectar.metaPuntos == 200) retoPicante = "<br><span style='font-size:12px; color:var(--accent-gold);'>🔥 RETO: Inventa una anécdota vergonzosa.</span>";
                                if (salaReconectar.metaPuntos == 300) retoPicante = "<br><span style='font-size:12px; color:var(--accent-gold);'>🌶️ RETO: Escribe sobre un gusto culposo.</span>";
                                if (salaReconectar.metaPuntos == 400) retoPicante = "<br><span style='font-size:12px; color:var(--accent-gold);'>😈 RETO PICANTE: Di algo malo que haya hecho.</span>";
                                if (j.nombre === victima.nombre) msj = "👑 ERES EL ELEGIDO (+20 PTS)";
                                else if (esAbog) msj = `😈 ABOGADO DEL DIABLO <br><span style="font-size:12px; color:#ff4757;">¡Inventa algo bueno sobre <b style="color:var(--accent-gold); text-transform:uppercase;">${victima.nombre}</b>!</span>`;
                                else msj += retoPicante;
                                socket.emit('faseEscritura', { msj: msj, esVictima: j.nombre === victima.nombre, esAbogado: esAbog, turno: salaReconectar.turnoActual, nombreVictima: victima.nombre });
                            }
                        }
                    }
                }
    ...
    socket.on('misionComodin', () => { if(miUsuario) progresarMision(miUsuario, 'comodines', socket.id); });

    // Panel de Administrador: Sumar monedas/VIP a usuarios
    socket.on('adminActualizarUsuario', (data) => {
        if (miUsuario !== 'soporte1' && miUsuario !== 'admin') return; // Seguridad
    ...
    socket.on('enviarSoporte', (data) => { 
        const usr = miUsuario || 'No_Logueado'; const fecha = new Date().toLocaleString(); 
    ...
    socket.on('obtenerSalas', () => { 
        const lista = Object.keys(salas).map(id => ({ 
            ...
            jugadores: salas[id].jugadores.length, espectadores: salas[id].espectadores.length, 
            enCurso: salas[id].juegoEnCurso, meta: salas[id].metaPuntos,
            tipoJuego: salas[id].tipoJuego // Para diferenciar Rumor Toxic de Exotic en el Lobby
        })); 
    ...
    socket.on('crearSala', (data) => { 
        ...
        salas[idSala] = { 
            ...
            tipoJuego: data.tipoJuego || 'rumor', 
    ...
    function unirseASala(socket, idSala, clave, rol = 'jugador') {
        ...
        db.get("SELECT * FROM usuarios WHERE usuario = ?", [miUsuario], (err, row) => {
            ...
            if (rol === 'espectador') {
                ...
                socket.emit('entradoEnSala', { nombreSala: sala.nombre, codigo: idSala, meta: sala.metaPuntos, rol: 'espectador', privada: !!sala.pass, tipoJuego: sala.tipoJuego });
            } else {
                soyEspectador = false;
                const index = sala.jugadores.findIndex(p => p.nombre === miUsuario);
                if (index !== -1) { 
                    sala.jugadores[index].id = socket.id; sala.jugadores[index].avatar = row.avatar || '👤'; sala.jugadores[index].avatarType = row.avatar_type || 'emoji'; sala.jugadores[index].vip = row.vip || 0; sala.jugadores[index].titulo = row.titulo || ''; sala.jugadores[index].marco = row.marco || ''; 
                } else { 
                    sala.jugadores.push({ id: socket.id, nombre: miUsuario, victorias: row.victorias || 0, avatar: row.avatar || '👤', avatarType: row.avatar_type || 'emoji', vip: row.vip || 0, titulo: row.titulo || '', marco: row.marco || '' }); esNuevo = true; 
                }
                ...
                socket.emit('entradoEnSala', { nombreSala: sala.nombre, codigo: idSala, meta: sala.metaPuntos, rol: 'jugador', privada: !!sala.pass, tipoJuego: sala.tipoJuego });
            }

            io.to(idSala).emit('actualizarSala', sala.jugadores, sala.espectadores.length); 
            io.emit('actualizarListaSalas');
            if (esNuevo && row.vip === 1 && rol === 'jugador') { io.to(idSala).emit('vipEntro', { nombre: miUsuario, entrada: row.entrada || '' }); }
        });
    }
    ...
    function salirDeSala(socket, desconexionForzada = false) {
        ...
        if (sala.juegoEnCurso && desconexionForzada && !soyEspectador) { const j = sala.jugadores.find(x => x.id === socket.id); if (j) j.id = 'desconectado'; miSalaID = null; return; }
        ...
        if (sala.jugadores.length === 0) { if (sala.timerVotacion) clearTimeout(sala.timerVotacion); delete salas[miSalaID]; } 
        else { if (eraLider) io.to(miSalaID).emit('notificacion', `👑 Nuevo líder: ${sala.jugadores[0].nombre}`); io.to(miSalaID).emit('actualizarSala', sala.jugadores, sala.espectadores.length); }
    ...
    socket.on('enviarMensajeChat', (texto) => {
        ...
        if (jugador) {
            let isVip = false; if(!soyEspectador) { const jVip = sala.jugadores.find(j => j.nombre === miUsuario); if(jVip && jVip.vip === 1) isVip = true; }
            const textoSeguro = String(texto).replace(/</g, "&lt;").replace(/>/g, "&gt;"); // Evitar inyección de HTML
    ...
    // --- INICIO Y FLUJO DE JUEGO PRINCIPAL ---
    socket.on('iniciarJuego', () => {
        ...
        // Si la sala es de EXOTIC, redirige a su lógica propia
        if (sala.tipoJuego === 'exotic') {
            iniciarRondaExotic(sala);
        } 
        // Si es RUMOR TOXIC, resetea puntajes y lanza el juego
        else {
    ...
    // MESA DE EXOTIC (Base para programar después)
    function iniciarRondaExotic(sala) {
        io.to(sala.id).emit('faseExotic', { msj: "¡Mesa de Exotic en construcción! Pronto habrá cartas." });
        io.emit('actualizarListaSalas');
    }
    ...
    function iniciarRonda(sala) {
        ...
        // Asignación de Roles: Víctima y Abogado del Diablo
        ...
        sala.puntajesPartida[victima.nombre] += 20; // Recompensa por ser la víctima
        ...
        // Selección del reto según dificultad
        let retoAleatorio = "";
        ...
        // Notificar a cada jugador su rol
    ...
    // Recepción de rumores y cobro de Power-Ups
        ...
        if(autor && !sala.rumores.find(r => r.autorNombre === autor.nombre)) {
            ...
            db.get("SELECT coins, toxic FROM usuarios WHERE usuario = ?", [autor.nombre], (err, row) => {
                ...
                db.run("UPDATE usuarios SET coins = coins - ?, toxic = toxic - ?, coins = coins + 5 WHERE usuario = ?", [costoCoins, costoToxic, autor.nombre], () => {
                    ...
                    sala.puntajesPartida[autor.nombre] += 5; // +5 Pts solo por participar enviando rumor
                    io.to(sala.id).emit('actualizarPuntajesPartida', sala.puntajesPartida);
                    io.to(miSalaID).emit('rumorRecibido', { recibidos: sala.rumores.length, total: sala.jugadores.length - 1 });

                    // Si todos enviaron su rumor, pasamos a Votación
                    if(sala.rumores.length >= sala.jugadores.length - 1) {
                        ...
                        sala.timerVotacion = setTimeout(() => { castigarElegido(sala); }, 30000); // 30s de límite
                    }
                });
            });
        }
    ...
    // Si la víctima tarda mucho, pierde puntos
    ...
    // Lógica cuando la víctima elige el rumor ganador de la ronda
    socket.on('elegirGanador', (originalIndex) => {
        ...
        if (ganador) {
            ...
            // Verifica si eligió al Abogado del Diablo
            if (nombreReal === sala.abogadoNombre) {
                ...
                if (ganador.aceptaReto) { ptsGanados += 10; msjEspecial += " 🔥 ¡El ganador completó el reto (+10 pts extra)!"; }
                ...
                if (usuariosOnline[nombreReal]) progresarMision(nombreReal, 'rumores', usuariosOnline[nombreReal]);
            }
    ...
    // Cierre de partida y entrega de recompensas
    function declararVictoriaFinal(sala, nombreGanador) {
        ...
        if (usuariosOnline[nombreGanador]) progresarMision(nombreGanador, 'victorias', usuariosOnline[nombreGanador]);
    ...
    socket.on('cambiarEntrada', (e) => { if(!miUsuario) return; db.run("UPDATE usuarios SET entrada=? WHERE usuario=?", [e, miUsuario], () => { socket.emit('notificacion', '⚡ Animación de entrada equipada.'); }); });
    
    // Funciones Sociales (Buscar amigos, enviar peticiones)
    ...
    // Desconexión limpia
    socket.on('disconnect', () => { salirDeSala(socket, true); delete usuariosConectados[socket.id]; if(miUsuario) delete usuariosOnline[miUsuario]; });

    // Middleware robusto sin errores en Node 24 para SPA (Single Page Applications)
    app.use((req, res, next) => { if (req.method === 'GET') { res.sendFile(path.join(__dirname, 'public', 'index.html')); } else { next(); } });
});
