...
io.on('connection', (socket) => {
        ...
        db.get("SELECT * FROM usuarios WHERE usuario = ?", [usuario], (err, row) => {
            db.all("SELECT resultado, fecha FROM historial WHERE usuario = ? ORDER BY id DESC LIMIT 10", [usuario], (eh, history) => {
                ...
                if (salaReconectar) {
                    ...
                    // Al reconectar, avisamos de qué juego era la sala
    ...
    // --- ENVIAR EL TIPO DE JUEGO A LA LISTA DE SALAS ---
    socket.on('obtenerSalas', () => { 
        const lista = Object.keys(salas).map(id => ({ 
            ...
            tipoJuego: salas[id].tipoJuego // 🤫 o 🔥
        })); 
        socket.emit('listaSalas', lista); 
    });
    
    // --- CREAR SALA CON TIPO DE JUEGO (RUMOR O EXOTIC) ---
    socket.on('crearSala', (data) => { 
        ...
        salas[idSala] = { 
            ...
            tipoJuego: data.tipoJuego || 'rumor', // AQUÍ SE GUARDA SI ES EXOTIC
    ...
    // --- INICIAR JUEGO (EL CEREBRO DIVIDIDO) ---
    socket.on('iniciarJuego', () => {
        ...
        // Si el juego es EXOTIC, llamamos a la función de Exotic
        if (sala.tipoJuego === 'exotic') {
            iniciarRondaExotic(sala);
        } 
        // Si es RUMOR TOXIC, lanzamos el juego original
        else {
    ...
    // Middleware robusto sin errores en Node 24
