...
db.serialize(() => {
    ...
    // Intentamos agregar la columna exotic_name si es una base de datos vieja (ignora el error si ya existe)
...
// ============================================================================
// 🔌 APIS DE RUMOR (INTACTAS PARA QUE EL JUEGO BASE FUNCIONE)
// ============================================================================
...
// ============================================================================
// 🔌 SOCKETS COMPARTIDOS (LOGIN, TIENDA, MISIONES)
// ============================================================================
io.on('connection', (socket) => {
        ...
        db.get("SELECT * FROM retos_pendientes WHERE id = ?", [id], (err, row) => {
            ...
            if(accion === 'aprobar') {
                db.run("INSERT INTO retos_comunidad (autor, texto, shots, modo) VALUES (?, ?, ?, ?)", [row.autor, row.texto, row.shots, row.modo], () => {
                    ...
                    db.run("INSERT INTO salon_fama (usuario, partidas, retos_creados, exotic_name) VALUES (?, 0, 1, ?) ON CONFLICT(usuario) DO UPDATE SET retos_creados = retos_creados + 1, exotic_name = ?", [row.autor, row.autor, row.autor], () => {
                        actualizarDatosFama();
                    });
                    
                    db.run("DELETE FROM retos_pendientes WHERE id = ?", [id]);
                    socket.emit('notificacion', '✅ Reto aprobado y enviado a la comunidad.');
    ...
    socket.on('proponerRetoComunidad', (data) => {
        db.run("INSERT INTO retos_pendientes (autor, texto, shots, modo) VALUES (?, ?, ?, ?)", [data.autor, data.texto, data.shots, data.modo], () => {
            socket.emit('notificacion', '⏳ Tu reto fue enviado y está esperando aprobación del Admin.');
    ...
    socket.on('entrarSalaExotic', ({ codigo, modo, usuario }) => {
        ...
        if (!salas[codigo]) salas[codigo] = { 
            tipoJuego: 'exotic', modo: modo || 'grupo', jugadores: [], turno: null, 
            temperatura: 0, quejas: [], pacto: [], reglaGlobal: { texto: "", turnos: 0 }
        };
        ...
        if (usuario.loginName) {
            db.run("INSERT INTO salon_fama (usuario, partidas, retos_creados, exotic_name) VALUES (?, 1, 0, ?) ON CONFLICT(usuario) DO UPDATE SET partidas = partidas + 1, exotic_name = ?", [usuario.loginName, usuario.exotic_name || usuario.loginName, usuario.exotic_name || usuario.loginName], () => {
                actualizarDatosFama();
            });
        }
    ...
    socket.on('pedirRetoExotic', ({ codigo, nivel }) => {
        ...
        db.get('SELECT texto FROM retos WHERE nivel = ? AND modo = ? ORDER BY RANDOM() LIMIT 1', [nivel, sala.modo], (err, dbReto) => {
            ...
            if (sala.modo === 'grupo' && sala.jugadores.length > 1) {
                // 🔥 EL ESCUDO: Verificamos que sala.turno no sea null antes de buscar su nombre
                const turnoActual = sala.turno ? sala.turno.nombre : null;
    ...
    // ============================================================================
    // 🧠 INICIO DE LÓGICA RUMOR TOXIC (PARTIDAS)
    // ============================================================================
